...
AllocationTable.prototype.setValue = function AllocationTable$setValue(c, v){
	var l = this.level;
	if(c in this.table[l]) delete this.table[l][c];
	this.table_inv[l][v] = true;
	this.table[l][v] = this.table[l][c] = v;
	while(this.table_inv.some(function(a){return a[this.table_min[l]];}, this)) this.table_min[l]++;
	return v;
};
...
AllocationTable.prototype.getValue = function AllocationTable$getValue(c){
	...
	for(var i=this.level; i>=0; i--){
		if(c in this.table[i]) return this.table[i][c];
	}
	return this.setValue(c, this.table_min[this.level]);
};

AllocationTable.prototype.pushState = function AllocationTable$pushState(){
	this.table.push({}); this.table_inv.push([]); this.table_min.push(this.table_min[this.level]);
	this.level++;
};

AllocationTable.prototype.popState = function AllocationTable$popState(){
	this.table.pop(); this.table_inv.pop(); this.table_min.pop();
	this.level--;
};
					...
					switch(a[1]){
						...
						default:
							if(a[1] in _macro_table){
								...
								_allocation_table.pushState();
								_parse.call(_macro[1], symbol_table);
								_allocation_table.popState();
