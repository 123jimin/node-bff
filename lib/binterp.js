...
var _default_printer = function default_printer(data, callback){
	callback();
}, _default_getchar = function default_getchar(callback){
	callback(0);
}, _default_debugger = function default_debugger(debug_info, callback){
	callback();
};

var BI = function BrainFuckInterpreter(option, print, getch, debug){
	...
	this._getch = getch || _default_getchar;
	this._debug = debug || _default_debugger;
};

var INIT_ARR_SIZE = 1024,
	INIT_STACK_SIZE = 16;
BI.prototype.execute = function execute(bir, done){
	var self = this;
