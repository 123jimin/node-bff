		...
		outer_loop: for(;;){
			...
			while(curr >= current_block[1].length){
				if(stack_top_ptr == 0) break outer_loop;
					...
					current_block = current_block[2].parent;
				}
			}
			...
			switch(curr_elem[0]){
				...
				case 'write':
					...
					break;
				case 'read':
					if(read_buffer_ind < read_buffer.length){
						arr[arr_ptr] = read_buffer[read_buffer_ind++];
					}else{
						self._read(function(data){
							read_buffer = data;
							read_buffer_ind = 0;
							if(data.length == 0 && self.eof != null) arr[arr_ptr] = self.eof;
							else arr[arr_ptr] = read_buffer[read_buffer_ind++];
							// Speed may not matter when data is read
				...
				case 'loop':
					...
					break;
				default:
					done_err("Unknown instruction: "+curr_elem[0]);
					return;
			}
		}
